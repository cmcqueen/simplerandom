...
* Implement target language's API idioms and/or existing random number
  generator API.
* Reasonable statistical properties of pseudo-random output.
...
[^3], it seems that the SHR3 algorithm defined in [^4] is flawed and
...
We still find KISS from [^5] useful mainly because it uses 32-bit
calculations for MWC, which can be more suitable for small embedded
systems. So we define KISS that uses a MWC based on [^6], but the Cong
and SHR3 from [^7].

From Pierre L'Ecuyer [^8] [^9], the Combined LFSR (Tausworthe) LFSR113
algorithm [^10] and LFSR88 (aka Taus88) have been implemented.
...
In `simplerandom.iterators`, the following pseudo-random number
generators are provided:

  Generator   Notes
  ----------- --------------------------------------------------------------------------------------------------------------
  `MWC1`      Two 32-bit MWCs combined. From [^11].
  `MWC2`      Very similar to `MWC1`, but slightly modified to improve its statistical properties.
  `Cong`      From [^12].
  `SHR3`      From [^13].
  `MWC64`     A single 64-bit multiply-with-carry calculation. From [^14].
  `KISS`      Combination of MWC2, Cong and SHR3. Based on [^15] but using Cong and SHR3 from [^16], and the modified MWC.
  `KISS2`     Combination of MWC64, Cong and SHR3. From [^17].
  `LFSR113`   Combined LFSR (Tausworthe) random number generator by L'Ecuyer. From [^18] [^19].
  `LFSR88`    Combined LFSR (Tausworthe) random number generator by L'Ecuyer. From [^20].

These generators are Python iterators, of infinite length (they never
raise `StopIteration`). They implement the `next()` method (`__next__()`
in Python 3.x) to generate the next random integer. All the generators
output 32-bit unsigned values, and take one or more 32-bit seed values
during initialisation/seeding.
...
newsgroup post [^21], as well as other unit tests.
...
[^4]: [Random Numbers for C: End, at last?](http://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.43.3639)\
    George Marsaglia\
    Newsgroup post, sci.stat.math and others, Thu, 21 Jan 1999

[^5]: [Random Numbers for C: End, at last?](http://www.iro.umontreal.ca/~simardr/rng/lfsr113.c)\
    George Marsaglia\
    Newsgroup post, sci.stat.math and others, Thu, 21 Jan 1999

[^6]: [Random Numbers for C: End, at last?](http://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.43.4155)\
    George Marsaglia\
    Newsgroup post, sci.stat.math and others, Thu, 21 Jan 1999
...
[^8]: Tables of Maximally-Equidistributed Combined LFSR Generators\
    Pierre L'Ecuyer\
    Mathematics of Computation, 68, 225 (1999), 261–269.

[^9]: Maximally Equidistributed Combined Tausworthe Generators\
    P. L'Ecuyer\
    Mathematics of Computation, 65, 213 (1996), 203–213.

[^10]: LFSR113 C double implementation\
    Pierre L'Ecuyer

[^11]: Random Numbers for C: End, at last?\
    George Marsaglia\
    Newsgroup post, sci.stat.math and others, Thu, 21 Jan 1999
...
[^15]: \`Random Numbers for C: End, at last?\`\_\_\
    George Marsaglia\
    Newsgroup post, sci.stat.math and others, Thu, 21 Jan 1999
...
[^18]: \`Tables of Maximally-Equidistributed Combined LFSR
    Generators\`\_\_\
    Pierre L'Ecuyer\
    Mathematics of Computation, 68, 225 (1999), 261–269.

[^19]: \`LFSR113 C double implementation\`\_\_\
    Pierre L'Ecuyer

[^20]: \`Maximally Equidistributed Combined Tausworthe Generators\`\_\_\
    P. L'Ecuyer\
    Mathematics of Computation, 65, 213 (1996), 203–213.

[^21]: \`Random Numbers for C: End, at last?\`\_\_\
    George Marsaglia\
    Newsgroup post, sci.stat.math and others, Thu, 21 Jan 1999
